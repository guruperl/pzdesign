	...
	<table class="table table-striped table-sm">
        ...
        <tbody>{{ with .Lists }}{{ range . }}
			<tr>
				...
				<td>{{.created}}</td>
				<td>
{{if eq .status "New"}}<a class="btn btn-sm btn-success" href="payment?action=update&status=Confirmed&payment_id={{.payment_id}}">确认</a> <a class="btn btn-sm btn-primary" href="payment?action=update&status=Failed&payment_id={{.payment_id}}">驳回</a>{{end}}
