...
<head>
  ...
  <meta name="description" content="W8M system administration sign-in">
  <meta name="theme-color" content="#0b1f33">
  <title>System Administration Sign-in | W8M</title>
...
<body class="w8m-public-account theme-internal">
  <header class="account-topbar">
    <div class="container">
      <a class="account-brand" href="/">W8M <small>System Administration</small></a>
      <nav class="account-topnav" aria-label="Administration sign-in navigation">
      ...
      <div class="account-card account-card-compact theme-internal">
        <aside class="account-context">
          <div class="account-context-copy">
            ...
            <p class="account-eyebrow">Internal Account</p>
            <h2>System Administration</h2>
            <p>This entry point is restricted to authorized system operations and maintenance personnel.</p>
          </div>
        </aside>
        ...
        <section class="account-form-panel">
          <div class="account-form-heading">
            ...
            <h1>System Administration Sign-in</h1>
          ...
          <form method="post" action="/goto/admin/e/{{ .LoginName }}">
            <input type="hidden" name="{{ .GoURIName }}" value="{{ .GoURI }}">
            <div class="account-field">
              <label for="admin-login-name">Administrator Username</label>
              <div class="account-control"><i class="fa fa-user-o" aria-hidden="true"></i><input id="admin-login-name" class="form-control" name="{{.Login}}" type="text" placeholder="Enter your administrator username" autocomplete="username" autofocus required></div>
            </div>
            <div class="account-field">
              <label for="admin-login-password">Password</label>
              <div class="account-control"><i class="fa fa-lock" aria-hidden="true"></i><input id="admin-login-password" class="form-control" name="{{.Password}}" type="password" placeholder="Enter your password" autocomplete="current-password" required></div>
            </div>
            <div class="account-field"><label for="admin-login-totp">Authenticator or Recovery Code</label><div class="account-control"><i class="fa fa-shield" aria-hidden="true"></i><input id="admin-login-totp" class="form-control" name="{{.TOTP}}" type="text" placeholder="Enter only when two-factor authentication is enabled" autocomplete="one-time-code"></div></div>
            <button type="submit" class="account-submit">Sign in to System Administration</button>
          </form>
        </section>
      </div>
    </div>
