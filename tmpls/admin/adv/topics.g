...
{{if eq .active "No"}}<a class="btn btn-sm btn-warning" href="adv?action=takedown&active=Yes&adv_id={{.adv_id}}">重新激活</a>{{end}}
				...
				<td><form method="post" action="manage" target="_blank"><input type="hidden" name="action" value="login_as"><input type="hidden" name="role" value="adv"><input type="hidden" name="adv_id" value="{{.adv_id}}"><button class="btn btn-sm btn-success" type="submit">进入账户</button></form></td>
			</tr>{{end}}{{end}}
		</tbody>
	</table>
</div>
