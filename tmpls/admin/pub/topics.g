...
{{if eq .active "No"}}<a class="btn btn-sm btn-warning" href="pub?action=takedown&active=Yes&pub_id={{.pub_id}}">重新激活</a>{{end}}
				...
				<td><form method="post" action="manage" target="_blank"><input type="hidden" name="action" value="login_as"><input type="hidden" name="role" value="pub"><input type="hidden" name="pub_id" value="{{.pub_id}}"><button class="btn btn-sm btn-success" type="submit">进入账户</button></form></td>
			</tr>{{end}}{{end}}
			<tr>
			...
			<td> </td>
			<td><input class="form-input" type=text name=domain placeholder="域名" /><input class="form-input" type=email name=email placeholder="账户邮箱" required /></td>
