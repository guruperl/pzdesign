...
<form method="post" action="midroute?action=update">
  ...
  <a class="btn btn-outline-primary" href="midroute?action=bidders&group_id={{$item.group_id}}">Bid Endpoints</a>
  <a class="btn btn-outline-primary" href="midroute?action=targets&group_id={{$item.group_id}}">Targets</a>
</form>

<hr>
<form method="post" action="midroute?action=delete" onsubmit="return confirm('Delete this route group? This action cannot be undone.');">
  <input type="hidden" name="group_id" value="{{$item.group_id}}">
  <button type="submit" class="btn btn-outline-danger">Delete Route Group</button>
</form>
