...
<th>MIME</th>
<th>时间</th>
<th colspan=4 class="text-right"><a class="btn btn-primary" href="#" data-title="添加广告组" data-href="item?action=startnew&campaign_id={{index .ARGS.campaign_id 0}}&campaign_md5={{index .ARGS.campaign_md5 0}}&campaign_name={{index .ARGS.campaign_name 0 | urlquery}}" id="startnewPopup">新建广告组</a></th>
</tr></thead>
<tbody>{{with .Lists}}{{range .}}
<tr {{if eq .active "New"}}class="warning"{{else if eq .active "Pause"}}class="danger"{{else}}{{end}}>
<td><a href="#" data-title="更新广告组：{{.item_name}}" data-href="item?action=edit&campaign_id={{index $.ARGS.campaign_id 0}}&campaign_md5={{index $.ARGS.campaign_md5 0}}&campaign_name={{index $.ARGS.campaign_name 0 | urlquery}}&item_id={{.item_id}}&item_md5={{.item_md5}}&item_name={{.item_name | urlquery}}" id="editPopup">{{.item_name}}</a></td>
