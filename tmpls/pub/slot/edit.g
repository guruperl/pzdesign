...
<option {{if eq $item.fl_mime "html"}}selected{{end}} value="html">页面</option>
<option {{if eq $item.fl_mime "image"}}selected{{end}} value="image">图片</option>
<option {{if eq $item.fl_mime "video"}}selected{{end}} value="video">视频</option></select>
    </div>
</div>

<div class="form-group row">
    <label for="inputPlatform" class="col-sm-2 col-form-label">所在平台:</label>
...
<div class="form-group row">
    <label for="inputPageLevel" class="col-sm-2 col-form-label">页面级别:</label>
