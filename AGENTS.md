...
1. `tabilet/memory-bank/product.md` — what W8M is, its roles, domain model, and invariants.
2. `tabilet/memory-bank/architecture.md` — layout, request flow, ownership, contracts.
3. `tabilet/memory-bank/tech-stack.md` — stack, dependencies, and every runnable command.
4. `tabilet/memory-bank/milestone.md` — active work index, acceptance, review procedure.
5. The `tabilet/memory-bank/status-<LANE><NN>.md` file for the milestone you are on.
...
`tabilet/docs/archive-M01.md` through `tabilet/docs/archive-M11.md` are **frozen evidence** at
commit `d67ea8ac35b0578db9674bfbb40fede56954cddf`. Current product and system
truth lives in `tabilet/memory-bank/product.md` and `tabilet/memory-bank/architecture.md`. Read
...
in `tabilet/memory-bank/milestone.md` before marking a milestone complete.

`tabilet/GOAL.md` is an optional protocol for running several milestones in order, and
`tabilet/memory-bank/suggested.txt` is disposable launch input for it. Both can be
